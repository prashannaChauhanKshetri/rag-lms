...
function SecuritySection() {
    ...
    return (
        <SectionCard title="Account Security" subtitle="Update your login password">
            ...
            <div className="space-y-4">
                {[
                    { label: 'Current Password', value: current, onChange: setCurrent, show: showCurrent, onToggle: () => setShowCurrent(p => !p) },
                    { label: 'New Password', value: newPw, onChange: setNewPw, show: showNew, onToggle: () => setShowNew(p => !p) },
                ].map(({ label, value, onChange, show, onToggle }) => (
                    <div key={label}>
                        <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1.5">{label}</label>
                        <div className="relative">
                            <input type={show ? 'text' : 'password'} value={value} onChange={e => onChange(e.target.value)} className="w-full px-3 py-2 pr-10 border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 transition" />
