    ...
    return (
        <SectionCard title="Appearance & Accessibility" subtitle="Customize how the platform looks and behaves">
            ...
            <div className="mb-6">
                ...
                <div className="flex gap-3">
                    <button onClick={() => handleTheme(false)} className={`flex-1 flex flex-col items-center gap-2 p-4 rounded-xl border-2 transition-all ${!isDark ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20' : 'border-gray-200 dark:border-gray-700 hover:border-gray-300 dark:hover:border-gray-600'}`}>
                        <Sun className={`w-5 h-5 ${!isDark ? 'text-blue-600' : 'text-gray-400'}`} />
                        <span className={`text-sm font-semibold ${!isDark ? 'text-blue-700 dark:text-blue-400' : 'text-gray-500'}`}>Light</span>
                    </button>
                    <button onClick={() => handleTheme(true)} className={`flex-1 flex flex-col items-center gap-2 p-4 rounded-xl border-2 transition-all ${isDark ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20' : 'border-gray-200 dark:border-gray-700 hover:border-gray-300 dark:hover:border-gray-600'}`}>
                ...
                <div className="flex gap-3">
                    {FONT_SIZES.map(({ value, label, sample }) => (
                        <button
                            ...
                            className={`flex-1 flex flex-col items-center gap-1.5 py-3 px-2 rounded-xl border-2 transition-all ${fontSize === value ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20' : 'border-gray-200 dark:border-gray-700 hover:border-gray-300 dark:hover:border-gray-600'}`}
                        >
                            <span className={`font-bold ${sample} ${fontSize === value ? 'text-blue-700 dark:text-blue-400' : 'text-gray-500 dark:text-gray-400'}`}>A</span>
                            <span className={`text-xs font-medium ${fontSize === value ? 'text-blue-600 dark:text-blue-400' : 'text-gray-400'}`}>{label}</span>
                        </button>
                    ))}
                </div>
            </div>
