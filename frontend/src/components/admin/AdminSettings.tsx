            ...
            <div className="space-y-4">
                ...
                <div>
                    ...
                    <div className="relative">
                        <input type={showCurrent ? 'text' : 'password'} value={current} onChange={e => setCurrent(e.target.value)} className="w-full px-3 py-2 pr-10 border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-purple-500 transition" />
                        <button type="button" onClick={() => setShowCurrent(p => !p)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600">{showCurrent ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}</button>
                    </div>
                </div>
                {/* New Password */}
                <div>
                    ...
                    <div className="relative">
                        <input type={showNew ? 'text' : 'password'} value={newPw} onChange={e => setNewPw(e.target.value)} className="w-full px-3 py-2 pr-10 border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-purple-500 transition" />
                        <button type="button" onClick={() => setShowNew(p => !p)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600">{showNew ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}</button>
                    </div>
                </div>
                {/* Confirm Password */}
                <div>
    ...
    return (
        <SectionCard title="Appearance & Accessibility" subtitle="Customize how the platform looks and behaves">
            ...
            <div className="mb-6">
                ...
                <div className="flex gap-3">
                    <button onClick={() => handleTheme(false)} className={`flex-1 flex flex-col items-center gap-2 p-4 rounded-xl border-2 transition-all ${!isDark ? 'border-purple-500 bg-purple-50 dark:bg-purple-900/20' : 'border-gray-200 dark:border-gray-700 hover:border-gray-300 dark:hover:border-gray-600'}`}>
                        <Sun className={`w-5 h-5 ${!isDark ? 'text-purple-600' : 'text-gray-400'}`} />
                        <span className={`text-sm font-semibold ${!isDark ? 'text-purple-700 dark:text-purple-400' : 'text-gray-500'}`}>Light</span>
                    </button>
                    <button onClick={() => handleTheme(true)} className={`flex-1 flex flex-col items-center gap-2 p-4 rounded-xl border-2 transition-all ${isDark ? 'border-purple-500 bg-purple-50 dark:bg-purple-900/20' : 'border-gray-200 dark:border-gray-700 hover:border-gray-300 dark:hover:border-gray-600'}`}>
                ...
                <div className="flex gap-3">
                    {FONT_SIZES.map(({ value, label, sample }) => (
                        <button
                            ...
                            className={`flex-1 flex flex-col items-center gap-1.5 py-3 px-2 rounded-xl border-2 transition-all ${fontSize === value ? 'border-purple-500 bg-purple-50 dark:bg-purple-900/20' : 'border-gray-200 dark:border-gray-700 hover:border-gray-300 dark:hover:border-gray-600'}`}
                        >
                            <span className={`font-bold ${sample} ${fontSize === value ? 'text-purple-700 dark:text-purple-400' : 'text-gray-500 dark:text-gray-400'}`}>A</span>
                            <span className={`text-xs font-medium ${fontSize === value ? 'text-purple-600 dark:text-purple-400' : 'text-gray-400'}`}>{label}</span>
                        </button>
                    ))}
                </div>
            </div>
